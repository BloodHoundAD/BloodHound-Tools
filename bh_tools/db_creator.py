...
class MainMenu(cmd.Cmd):
    def __init__(self):
        ...
        self.first_names = pickle.loads(
            pkgutil.get_data(__name__, 'data/first.pkl')
        )
        self.last_names = pickle.loads(
            pkgutil.get_data(__name__, 'data/last.pkl')
        )
...
def main():
    ...
    if os.environ.get("NEO4J_ENCRYPTION"):
        NEO4J_ENCRYPTION = bool(int(os.environ["NEO4J_ENCRYPTION"]))

    if NEO4J_URL and NEO4J_USERNAME and NEO4J_PASSWORD:
        menu.url = NEO4J_URL
        menu.username = NEO4J_USERNAME
        menu.password = NEO4J_PASSWORD
        menu.use_encryption = NEO4J_ENCRYPTION
        menu.do_clear_and_generate(None)
